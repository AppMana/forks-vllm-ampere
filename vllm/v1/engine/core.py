    ...
    def step_with_batch_queue(
        ...
        if self.scheduler.has_requests():
            ...
            if scheduler_output.total_num_scheduled_tokens <= 0:
                # In pipeline parallelism, the scheduler can have live requests
                # whose next local work is blocked on already-submitted downstream
                # batches (common with prefix-cache + chunked prefill across the PP
                # chain). Do NOT enqueue a no-forward placeholder into the PP batch
                # queue: it consumes a queue slot and stalls the chain (a rank blocks
                # forever waiting for a forward that never happens). Drain the queue
                # instead. Ported from pre-rebase 4ab2b13c3 "Skip empty PP scheduler
                # turns".
                if not batch_queue:
                    return None, False
            else:
