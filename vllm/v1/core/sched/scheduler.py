...
class Scheduler(SchedulerInterface):
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        # DeepSeek V4 PP MTP verification is only correctness-safe for singleton
        # decode steps in this branch. If a draft generated by a singleton step
        # is later scheduled into a multi-request PP batch, the expanded qlen>1
        # verifier path can perturb target-token logits even when every draft
        # token is force-rejected. Drop draft verification from those
        # multi-request batches and verify only the target next token.
        if self.use_pp and scheduled_spec_decode_tokens and len(num_scheduled_tokens) > 1:
            for req_id, spec_token_ids in list(scheduled_spec_decode_tokens.items()):
                num_scheduled_tokens[req_id] -= len(spec_token_ids)
                if num_scheduled_tokens[req_id] <= 0:
                    raise AssertionError(
                        "dropping PP MTP draft tokens removed all scheduled "
                        f"tokens for request {req_id}"
                    )
